...
export function EmployeeNav({ currentPath }: EmployeeNavProps) {
  ...
  return (
    <>
          ...
          {mobileNavItems.map((item) => {
            ...
            return (
              ...
              >
                {active && (
                  <motion.div
                    layoutId="employeeActiveTabMobile"
      ...
      {/* Desktop Side Nav - Slim version for employees */}
          ...
          {desktopNavItems.map((item) => {
            ...
            return (
              ...
              >
                {active && (
                  <motion.div
                    layoutId="employeeActiveTabDesktop"
                    className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 bg-primary rounded-full"
                    transition={{ type: "spring", stiffness: 500, damping: 30 }}
                  />
                )}
                <Icon className={cn("h-5 w-5", active && "text-primary")} />
                <span className={cn("font-medium flex-1", active && "text-primary")}>{item.label}</span>
                <kbd
                  className={cn(
